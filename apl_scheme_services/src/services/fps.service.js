...
class FPSService {
  ...
  async getAll(queryParams) {
    try {
      ...
      // Get total count
      const countQuery = `
        SELECT COUNT(*) FROM m_fps f
        LEFT JOIN m_afso a ON f.afso_code = a.afso_code
        ${whereClause}
      `;
      ...
      const dataQuery = `
        SELECT f.id, f.fps_code, f.afso_code, f.description_en, f.description_ll, 
               f.is_active, f.created_at, f.created_by, f.modified_at, f.modified_by,
  ...
  async getAllActiveByAFSO(afsoCode) {
    try {
      const query = `
        ...
        FROM m_fps 
        WHERE is_active = true AND afso_code = $1
